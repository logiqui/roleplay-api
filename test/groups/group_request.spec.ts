...
test.group('Group Request', (group) => {
  ...
  test('it should return 404 when provide an unexisting group', async (assert) => {
    const master = await UserFactory.create()
    const group = await GroupFactory.merge({ master: master.id }).create()
  ...
  test('it should return 404 when provide an unexisting group request', async (assert) => {
    const master = await UserFactory.create()
    const group = await GroupFactory.merge({ master: master.id }).create()
