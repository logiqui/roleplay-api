...
test.group('User', (group) => {
  test('it should create an user', async (assert) => {
    const userPayload = {
      ...
      username: 'test',
      password: 'test',
      avatar: 'https://github.com/logiqui.png'
    }
  ...
  test('it should return 422 when provide an invalid password', async (assert) => {
    const { body } = await supertest(BASE_URL)
      .post('/users')
      .send({ email: 'test@test.com', username: 'test', password: 'tes' })
      .expect(422)
  ...
  test('it should update an user', async (assert) => {
    ...
    const avatar = 'https://github.com/logiqui.png'
  ...
  test('it should return 422 when provide an invalid password', async (assert) => {
    ...
    const { body } = await supertest(BASE_URL)
      .put(`/users/${id}`)
      .send({ email, avatar, password: 'tes' })
      .expect(422)
